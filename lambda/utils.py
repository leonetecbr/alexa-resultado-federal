...
class Federal():
    ...
    def generateUrl(self):
        ...
        response = requests.get(
            'http://www.loterias.caixa.gov.br/wps/portal/loterias/landing/federal', headers=self.h, cookies=self.c)

        txt = response.text

        p1 = re.search(
            '\<link id="com\.ibm\.lotus\.NavStateUrl" rel="alternate" href="(.*)" \/\>', txt)

        p2 = re.search(
            '\<input type="hidden" value="(.*)" ng\-model="urlBuscarResultado" id="urlBuscarResultado" \/\>', txt)

        url = 'http://www.loterias.caixa.gov.br' + \
            p1.group(1)+p2.group(1)+'?timestampAjax='
